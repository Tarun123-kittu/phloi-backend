...
exports.get_all_verified_hotels = async (req, res) => {
    try {
        ...
        let allVerifiedHotels = await hotelModel.find({ adminVerified: true, paymentStatus: "completed" ,  subscriptionEndDate: { $gte: new Date() }}).select("establishmentName establishmentType address images customerServiceNumber openCloseTimings").lean()
